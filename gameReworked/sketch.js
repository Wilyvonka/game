...
soundeffects
give asteroids gravity or collition detection?
make them a possible strategy for winning
let user pick window size based on specs
map-color for newly spawned objects
*/
...
function draw() {
	if (highscore >= year()) {
		...
		for (let i = 0; i < asteroids.length; i++) {
			if (asteroids[i].collided(planet)) {
...
function Firework() {
	this.firework = new Particle(planet.pos.x, planet.pos.y, true, random(-PI / 2, PI / 2));
	this.exploded = false;
	this.particles = [];
...
class Asteroid {
	...
	update() {
		...
		translate(0, 0)
		angleMode(RADIANS)
		//this.acc.set(-this.dist.div(pow(gravity, 2)))
		//this.acc.set(-gravity / (this.dist.x * 1000), -gravity / (this.dist.y * 1000)
...
class Planet {
	...
	render() {
		...
		noStroke()
		if (highscore < year()) {
			ellipse(ship.pos.x, ship.pos.y, 10)
		}
		pop()
	}
}
